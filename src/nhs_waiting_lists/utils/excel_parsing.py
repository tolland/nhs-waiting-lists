...
def process_op_excel(data, year, yr_next, _suffix):
    ...
    df= df.rename(columns={'Code': 'organisation_code'}) if 'Code' in df.columns else df
    ...
    df = df.rename(columns=EXCEL_OP_MAPS,
                   # errors='ignore'
                   )
    # print(df.columns)
    ...
    check_valid_pairs(OP_MEASURE_TYPE_VALID_PERMS, ["measure_type", "measure"], df, raise_on_error=False)

    mask_set = df.apply(
        lambda row: (row['measure_type'], row['measure']) in set(OP_MEASURE_TYPE_VALID_PERMS),
        axis=1
    )
    df_filtered = df[mask_set].iloc[:, :]
    # confirm that the filtered set is valid, raise if not
    check_valid_pairs(OP_MEASURE_TYPE_VALID_PERMS, ["measure_type", "measure"], df_filtered)

    valid_df = pd.DataFrame(OP_MEASURE_TYPE_VALID_PERMS, columns=["measure_type", "measure"])
    ...
    expected = (
        ...
        .merge(pd.DataFrame({"organisation_code": df_filtered["organisation_code"].unique(), "key": 1}), on="key")
        .drop(columns="key")
    )
    print(f"expected: {len(expected)}")

    # select the actual triplets
    actual = df_filtered[["organisation_code", "measure_type", "measure"]].drop_duplicates()
    ...
    if missing.empty:
        ...
        print(f"⚠️ Missing {len(missing)} combinations across {missing['organisation_code'].nunique()} codes.")
        print(missing.groupby("organisation_code")[["measure_type", "measure"]]
              .apply(lambda g: g.values.tolist())
              .head(10))  # show a sample
...
def normalize_measure2(value: str) -> str:
    """
